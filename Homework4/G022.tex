...
Therefore, for a general solution, we have two options, either $\max_j (\#(c_1 \cap l_j)) = 3$ and $\max_j (\#(c_2 \cap l_j)) = 0$ or $\max_j (\#(c_1 \cap l_j)) = 2$ and $\max_j (\#(c_2 \cap l_j)) = 1$.
The first one, does not make sense since it would mean that we have an empty cluster.

\subsection*{General Solution Using 2 Clusters}
